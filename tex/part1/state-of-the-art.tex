...
%	    where the authors propose a \textit{D-GEX}, DL regression architecture trained to predict the expression of a number of target genes. Dropout
  ...
  In both cases we will try to infer the properties of the probability distribution $p(\bm{x})$ from which the collected data are assumed to be drawn.
    ...
    In ML jargon, with \textit{cluster analysis}, \aka \textit{data segmentation} or \textit{clustering}, one typically means the class of unsupervised learning algorithm which aim at grouping together similar objects, by some definition of \textit{similarity}.
        
    This task is fairly common in biomedical studies, where cluster analysis can be used, for instance, to perform patients stratification or to find groups of correlated biological measures (such as gene expression, blood exams, \etc). In this section an overview on the most popular clustering algorithms is presented.\footnote{a good reference for the clustering algorithms proposed in literature is section \textit{2.3. Clustering} of the \sklearn documentation \url{http://scikit-learn.org/stable/modules/clustering.html}.}

    \subsubsection{The $k$-means algorithm} \label{sec:k-means}
    The $k$-means algorithm is, for cluster analysis, what $k$-nearest neighbor and OLS are for supervised classification and regression, respectively: the most intuitive strategy.

	Given $n$ samples $\bm{x}_i \in \mathbb{R}^d$, the goal of $k$-means is to partition the data into $K$ non-overlapping groups clusters. Intuitively, samples falling in the same cluster will be more similar one another.
    ...
    Fixed the number of cluster, the output of the $k$-means algorithm is a set of cluster centroids as well as the assignment of each sample to a single cluster.
    ...
    \end{equation}
    Therefore, our goal is to find the centroids $\bm{\mu}_k$ for $k=1,\dots,K$ that minimize $J(\bm{\mu})$.
    ...
    Fixing $r_k(\bm{x})$ for all $k$, the objective function in Equation~\eqref{eq:kmeans} is quadratic in $\bm{\mu}$, therefore the minimization problem can be easily solved setting its derivative to zero (see Equation~\eqref{eq:kmeans_solved}).
    ...
    The $k$-means algorithm starts with random centroid. Then, iteratively assigns the points to the cluster having the nearest centroid and updates the centroids position. At each step the value of $J(\bm{\mu})$ is reduced, therefore the convergence of $k$-means is assured. Nevertheless, it may converge to a local minimum, hence leading to a suboptimal solution.
...
%    \end{enumerate}
	...
	Even though this initial seeding comes with an computational time at the beginning, then the $k$-means algorithm converges super fast actually leading to a decreased overall computational time. Moreover, the achieved solution has considerably lower final cluster reconstruction, compared to standard $k$-means.
	...
	Our hope is then to achieve a graph structure where the edges between points of a group is high and it is low between points belonging to different groups.
	...
	\end{algorithmic}
	\end{algorithm}
	Normalized version this algorithm is identical but the formulation of $L$ which is substituted by the normalized graph Laplacian.
