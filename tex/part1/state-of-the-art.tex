...
%    \end{enumerate}
  ...
  As opposed to the model parameters, which are learned from data, hyperparameters are user-defined.
  ...
  In this case, the model weights $w_j$ (for $j=1,\dots,d$) of the input variables are learned from training data, are the \textit{parameters} of the model, whereas the regularization parameter $\lambda$ is the only hyperparameter of the model.
  
  
  Using a robust protocol for model selection and evaluation is often the key to successfully run some data science challenge.
