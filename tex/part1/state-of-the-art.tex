	...
	%    The regularization penalty $R(f)$ imposes stability on the expected function exploiting the available prior knowledge on the problem~\cite{tikhonov1963solution}. More details on $R(f)$ will be given in the next section.
	    ...
	    In the following sections an overview of the most popular regularization methods is presented.
			...
			\begin{equation} \label{eq:ols}
				\bm{\hat w}_{OLS} = \argmin_{\bm{w} \in \mathbb{R}^d} \frac{1}{n} \sum_{i=1}^n (\bm{x}_i^T\bm{w} - y_i)^2 = \argmin_{\bm{w} \in \mathbb{R}^d} \frac{1}{n} \norm{X\bm{w} - \bm{y}}_2^2
			\end{equation}

			The minimization problem in Equation~\eqref{eq:ols} is convex and differentiable, hence its solution can be achieved both in closed-form as
			$$
				\bm{\hat w}_{OLS} = (X^TX)^{-1}X^T\bm{y}
			$$
			and by iterative minimization routines such as (stochastic) gradient descent-like algorithms~\cite{boyd2004convex}. A pictorial representation of the solution of an ordinary least squares problem can be seen in Figure~\ref{fig:w_ols}.

			OLS is probably the most na\"ive regression strategy, nevertheless it is widely adopted in several studies. For instance, let's see what happens when we use the OLS model on a real regression problem.
			For this example, and the following ones, we take into account the dataset $\mathcal{D}_{\text{aging}} = \{(\bm{x}_i, y_i)\}_{i=1}^{n=111}$ where $\bm{x}_i \in \mathbb{R}^{12}$ and $y_i \in \mathbb{R}$ ($\forall i=1,\dots,111$). For the sake of this discussion, a  thorough description of $\mathcal{D}_{\text{aging}}$ is irrelevant\footnote{this regression problem is widely described and analyzed in Chapter~\ref{chap:frassoni}.}, we will simply consider the $d=12$ variables as predictors of a continuous outcome $y$ and we will look for a linear input-output relationship.
			In order to do that, we randomly split $\mathcal{D}_{\text{aging}}$ in two, hence obtaining a training and test sets of, $n_{\text{tr}}=74$ and $n_{\text{ts}}=37$ samples respectively.

			\begin{figure}[!h]
				\includegraphics[width=\textwidth]{part1/coefs_ols.png}
				\caption{A pictorial representation of the vector $\bm{\hat w}_{\text{OLS}}$ obtained fitting an OLS model on $74$ randomly selected training samples of $\mathcal{D}_{\text{aging}}$. Variables associated with a positive (\ie directly proportional to the output) and a negative (\ie inversely proportional) weight are represented in blue and red, respectively.} \label{fig:coefs_ols}
		  \end{figure}


			We have already pointed out how learning predictive functions only by minimizing a loss function may lead to suboptimal solutions; therefore in the following sections we will explore pro and cons of using different penalties.

			\subsubsection{Ridge regression}
			In its original proposition, \textit{ridge regression}~\cite{hoerl1970ridge} is defined as a least squares problem penalized by an $\ell_2$-norm on the regression coefficients, see Equation~\eqref{eq:l2}.

			\begin{equation}\label{eq:l2}
				\mathcal{R}_{\mbox{$\ell_2$}}(\bm{w}) = \sum_{j=1}^d (w_j)^2 = ||\bm{w}||_2^2
			\end{equation}
			...
			$$
			where $I$ is the $d \times d$ identity matrix. An estimate for the ridge coefficients can of course also be obtained with gradient descend-like optimization routines~\cite{boyd2004convex}.

			\begin{figure}[!h]
				\includegraphics[width=\textwidth]{part1/coefs_ridge.png}
				\caption{A pictorial representation of the vector $\bm{\hat w}_{\ell_2}$ obtained fitting a Ridge model on $74$ randomly selected training samples of $\mathcal{D}_{\text{aging}}$. Variables associated with a positive (\ie directly proportional to the output) and a negative (\ie inversely proportional) weight are represented in blue and red, respectively.} \label{fig:coefs_ridge}
		  \end{figure}

			Although mainly used for regression problems, as the square loss can be written as a margin loss function
...
%			For instance, in~\cite{kratsch2014} the authors propose a ridge regression-based method to estimate ancestral characters from phylogenetic trees, while in~\cite{bovelstad2007} ridge regression is used to perform survival prediction from gene expression data.
			...
			%In the first case,
			\todo{On the other hand, this method is not commonly used to solve classification problems, where penalized Logistic regression is usually preferred (see Section~\ref{sec:logistic_regression}).}

			Given its straightforwardness, ridge regression was rediscovered and proposed several times under several different names, among which we recall \textit{Tikhonov regularization}~\cite{tikhonov1963solution}, \textit{weight decay}~\cite{krogh1992simple} \todo{or simply $\ell_2$-regularization~\cite{??}}.
%			and \textit{Regularization Network}~\cite{evgeniou2000regularization}.
			...
			\begin{figure}[!h]
				\includegraphics[width=\textwidth]{part1/coefs_lasso.png}
				\caption{A pictorial representation of the vector $\bm{\hat w}_{\ell_1}$ obtained fitting a Ridge model on $74$ randomly selected training samples of $\mathcal{D}_{\text{aging}}$. Variables associated with a positive (\ie directly proportional to the output) and a negative (\ie inversely proportional) weight are represented in blue and red, respectively.} \label{fig:coefs_lasso}
		  \end{figure}
			...
			\begin{figure}[!h]
				\includegraphics[width=\textwidth]{part1/coefs_enet.png}
				\caption{A pictorial representation of the vector $\bm{\hat w}_{\ell_1\ell_2}$ obtained fitting a Ridge model on $74$ randomly selected training samples of $\mathcal{D}_{\text{aging}}$. Variables associated with a positive (\ie directly proportional to the output) and a negative (\ie inversely proportional) weight are represented in blue and red, respectively.} \label{fig:coefs_enet}
		  \end{figure}
			...
			\begin{figure}[!h]
				\includegraphics[width=\textwidth]{part1/l1l2_norms_scatterplot.png}
