...
The term \textit{Machine Learning} (ML) first appeared in the late '50s in the field of computer science and it is now becoming a buzzword used in several contexts spanning from particle physics and astronomy to medicine and social sciences~\cite{service2017ai}.
With a simple search on Google Trends\footnote{ Source: \url{https://trends.google.com} (last visit 2017-09).} it is possible to roughly quantify the pervasiveness of this term on the Internet in the last few years. From Figure~\ref{fig:google_trend_ML} we can see that the interest toward both the terms \textit{machine learning} and \textit{data science} is growing, with the first consistently superior to the second.
...
\begin{enumerate}
  ...
  \item Trevor Hastie, a well-known applied statistician, in a famous seminar\footnote{ Part of Data Science @ Stanford Seminar series. Source: \url{https://goo.gl/UFgqxU} (last visit 2018-01).} held in October 2015 at the Stanford University, gave the following three definitions.
...
%	    The most widely adopted common strategy to build predictive models out of noisy data is called \textit{regularization}.
      In ML literature, \textit{regularization} is the most important countermeasure to overfitting and it is widely adopted, under several forms, to build predictive models out of noisy data.
	    ...
	    The original contribution of this thesis mainly relies on the application of data science and ML concepts to noisy domains. Therefore, regularization strategies are of primary interest in this discussion.
	    For each learning algorithm described, particular emphasis will be put on the relevant \textit{regularization} strategies.
	    ...
	    \end{equation}
			...
			Figure~\ref{fig:regression_fit}~\footnote{ Source \url{https://losc.ligo.org/events/GW150914/} (last visit 2018-01).}
			and
		  Figure~\ref{fig:classification_fit}~\footnote{ Source \url{https://archive.ics.uci.edu/ml/datasets/HTRU2} (last visit 2018-01).}, respectively.
	...
	%    The regularization penalty $R(f)$ imposes stability on the expected function exploiting the available prior knowledge on the problem~\cite{tikhonov1963solution}. More details on $R(f)$ will be given in the next section.
	    ...
	    In the following sections an overview of the most popular regularization methods is presented.
			...
			OLS is probably the most na\"ive prediction strategy, nevertheless it is widely adopted in several studies. Let us see what happens when we use the OLS model on a real regression problem.
...
%			For instance, in~\cite{kratsch2014} the authors propose a ridge regression-based method to estimate ancestral characters from phylogenetic trees, while in~\cite{bovelstad2007} ridge regression is used to perform survival prediction from gene expression data.
			...
			Let us see what happens when this method is applied to a real regression problem. For ease of comparison, we take into account the dataset $\mathcal{D}_{\text{aging}}$, introduced in Section~\ref{sec:ols}.
		  ...
		  \end{equation}
			...
			Let us see what happens when the Elastic-Net model is applied to an actual regression problem. As usually, we tackle the aging task (introduced in Section~\ref{sec:ols}).
...
%			\subsubsection{Other sparsity-inducing penalties} \label{sec:other_sparsity}
      ...
      The sparse logistic regression minimization problem can be efficiently solved either by proximal forward-backward splitting strategies, such as FISTA~\cite{beck2009fast}, or by coordinate descent~\cite{wu2008coordinate}.
			...
			As for the square loss-based methods, let us apply ($\ell_2$-penalized) SVR to the aging problem (introduced in Section~\ref{sec:ols}).
			...
			For ease of reading, all the regularization methods presented so far were focused on learning linear input-output relationships $y = \bm{x}^T\bm{w}$. They can all be extended to the nonlinear case exploiting an elegant mathematical approach called: the \textit{kernel trick}.
...
%			This task can be pursued with different strategies.
			When the problem cannot be effectively solved with standard kernels, new ones can be easily built.
			For instance, a simple way to construct a new kernel is to devise an explicit feature map $\phi(\bm{x})$. Otherwise, new kernels can be built by combining simpler kernel used as building blocks. Furthermore, we can argue that evaluating $k(\bm{x}_i, \bm{x}_j)$ can be compared to a measure of \textit{similarity} between the two input samples. Therefore, when a particular notion of \textit{similarity} between points is available and this can be exploited to design \textit{ad hoc} kernels.
			Exploiting prior knowledge on the problem to design new kernels is known as kernel engineering~\cite{shawe2004kernel, bishop2006pattern}.
%			For instance, a very natural way to compare two strings is to count how many substrings of a given length they share. This can be formalized in the so-called \textit{spectrum kernel}		
...
%			Tackling real-world problems with ad hoc kernels used to be the way to go up the first decade of the years $2000$. After that, with the advent of deep learning (see Section~\ref{sec:deep_learning}) the conce
%			
...
%		\end{table}	
		...
		Let us take into account the usual setting: $\mathcal{D} = \{(\bm{x}_i, y_i)\}_{i=1}^n = (X, \bm{y})$. Even when the number of nodes is fixed, defining at the same time: which variables to split, their splitting threshold and the predictive value is computationally unfeasible as it has a combinatorially large number of possible solutions. Therefore, starting from the root note, \ie the whole input space, and fixing a maximum tree depth, we resort to a greedy approximation.
...
%	    Decision trees are easily interpretable models that recursively partitions the training data into subsets, based on the test of a single feature value at each split (or node). At each iteration, the feature that yields the best split in terms of a pre-selected metric (Gini impurity, information gain or variance reduction) is chosen to create a new node. Decision trees is that they tend to not perform well in practice, which led to the introduction of random forests in 2001~\cite{breiman2001random}.
	    ...
	    Moreover, increasing the number of trees in the ensemble, RF does not overfit the data. Therefore, this parameter is typically chosen to be \textit{as large as possible}, consistently with the available hardware and computational time~\cite{breiman2001random}.
%	    By contrast, {\it boosted decision trees} are made by building an iterative collection of decision trees, trained by giving more importance to training examples that were incorrectly classified by the previous trees. \todo{Add use cases for boosted decision trees. $\dots$}
		...
		In practice, feature selection schemes that rely on RF may be unstable~\cite{kursa2014robustness}, therefore feature selection stability measures must be adopted to avoid drawing inconsistent conclusions.
	    ...
	    Let us see what happens when GB is used to tackle the aging problem (see Section~\ref{sec:ols}).
	    ...
	    As already seen for RF, we shall investigate the effect of increasing boosting iterations for GB. As shown in Figure~\ref{fig:gb_increasing}, after approximately $50$ boosting iterations the model reaches perfect overfit of the training set, \ie zero training error (Figure~\ref{fig:gb_mae}) and $100\%$ trained explained variance (Figure~\ref{fig:gb_ev}).
...
%	    \todo{consider adding boosting method for cox models...}
		...
		This section is definitely not an attempt to summarize the heterogeneous plethora of the DL methods presented in literature so far. Indeed, even a dedicated thesis may not be enough to accomplish this task.
...
%	    where the authors propose a \textit{D-GEX}, DL regression architecture trained to predict the expression of a number of target genes. Dropout
		...
		Let us see what happens when a relatively simple MLP is applied to the aging problem (see Section~\ref{sec:ols}). The experimental setup is the same used for RF in Section~\ref{sec:random_forests}, where the only parameter optimized via grid-search cross-validation is the weight decay constant.
		...
		Long-Short Term Memory Networks (\ac{LSTM}s) belong to the class of Recurrent Neural Networks (\ac{RNN}s)~\cite{goodfellow2016deep}.
...
%	\todo{discuss a bit the variable selection stability}
  ...
  For instance, let us say that a kid growing with a Brittany dog at home visit some friend having a Bernese Mountain Dog.
...
%    It is basically what $k$-nearest neighbor and OLS are for supervised classification and regression, respectively.
    ...
    We can argue that, given the best centroids, the samples can be simply associated to the cluster with the nearest centroid. So, let us focus on the problem of finding $\bm{\mu}_k$.
...
%    \end{enumerate}
	...
	Our aims is then to achieve a graph structure where the edges between points of the same group is high and it is low between points belonging to different groups.

	Therefore, let us consider a set of points $\mathcal{D}=\{\bm{x}_i\}_{i=1}^n$, the steps of the unnormalized spectral algorithm are summarized as follows\footnote{ Normalized version this algorithm is identical but the formulation of $L$ which is substituted by the normalized graph Laplacian~\cite{von2007tutorial}.}.
	\begin{algorithm}[h!]
	\begin{algorithmic}[1]
  ...
  The previous chapters present an overview of ML techniques and algorithms that are most relevant with the data science challenges presented in the second part of this thesis.
  ...
  In order to better understand this difference, let us see a practical example: a regularized least square approach (such as Ridge or Lasso).
  ...
  Nevertheless, different statistically sound techniques to perform hyperparameters optimization were presented over the years . At the time of writing, model selection is a hot-topic in ML research. It often happens that several ML solutions adopted to tackle data science competitions\footnote{ Such as the ones hosted on the Kaggle website: \url{https://www.kaggle.com} (last visit 2018-01).} are based on the same model. What really makes the difference, and defines the most competitive prediction strategy, is to adopt an efficient and effective model selection strategy.
...
\section{Hardware and software solutions} \label{sec:implementation}
...
All the data science challenge described in the second part of this PhD thesis are tackled with Python-based software solutions. In particular, the numerical computations are carried out by {\sc NumPy}\footnote{ Source: \url{http://www.numpy.org} (last visit 2018-01).} and {\sc SciPy}\footnote{ Source \url{https://www.scipy.org} (last visit 2018-01).}, while the data management is left to {\sc pandas}\footnote{ Source \url{https://pandas.pydata.org} (last visit 2018-01).}. Moreover, for the vast majority of standard ML models (such as generalized linear models or ensemble learning strategies) I took advantage of \sklearn~\cite{scikit-learn}\footnote{ Source \url{http://scikit-learn.org} (last visit 2018-01).}, which is one of the most popular ML libraries available online.
