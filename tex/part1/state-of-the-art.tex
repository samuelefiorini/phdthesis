...
Indeed, ML is currently experiencing a renewed wave of interest. Several new technologies, both software and hardware, are developed tailored for ML applications. The number of ML practitioners and researcher is growing fast and the problems that now are tackled with ML is rapidly evolving. Therefore, a description of the current technological state of the art is completely pointless as it would become outdated in no time.

Nevertheless, what may be more interesting, as it may last longer, is to describe the philosophy that the most appreciated ML software solutions are currently adopted.
The vast majority of data science software and libraries presents present two faces: a high level interface (mainly developed for Python, R, Julia or Torch) while delegating most of the heavy computations to optimized algorithms written, for instance, in C++, Fortran.

Moreover....

As brilliantly pointed out by Fran\c{c}ois Chollet in his book~\cite{chollet2018deep}:
\begin{displayquote}
