...
%	    where the authors propose a \textit{D-GEX}, DL regression architecture trained to predict the expression of a number of target genes. Dropout
  ...
  In both cases we will try to infer the properties of the probability distribution $p(\bm{x})$ from which the collected data are assumed to be drawn.
    ...
    $$
    where $d(a,b)$ is some distance measure. In case $d(a,b)$ is the squared Euclidean distance the objective function can be rewritten as in Equation~\eqref{eq:kmeans}.
    ...
    We may observe that, given the best centroids, the samples will be associated to the cluster with the nearest centroid. So, let's focus on the problem of finding the centroids.
    
    \todo{starting from random centroids do as page 425 of bishop}
