...
%Hence, for each index, the three risk categories are summarized in Table~\ref{tab:lbgihbgi}.

%To estimate LBGI, the BG readings are first transformed to obtain BG symmetric, zero-centered ranges as in~\cite{kovatchev1997symmetrization}, see Equation~\eqref{eq:preproc}.
%
%\begin{equation}\label{eq:preproc}
%	\text{Transformed BG} = 1.509 \cdot \big[ \left[\log (\text{BG})\right]^{1.084} - 5.381 \big]
%\end{equation}
%
%Hence, if $\text{Transformed BG} < 0$ then \mbox{$\text{Risk(BG)} = 10 \cdot \text{Transformed BG}^{2}$} else $\text{Risk(BG)} = 0$. Finally, LBGI is computed as the mean of these risk values across all BG readings.
