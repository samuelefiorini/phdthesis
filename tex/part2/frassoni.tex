...
\begin{itemize}
	...
	ATP is heavily involved in different cellular aerobic respiration pathways, therefore its intracellular concentration, measured in $\text{mM}/\text{m}$, is an important molecular biomarker to evaluate the energetic state of a cell.
	
	\item[] \textbf{AMP} This molecule is one of the main derivatives of ATP. In fact, AMP ca be obtained as a side product of the ATP synthesis process or when two phosphate groups are removed from ATP, releasing energy that can be transferred to other molecules to trigger some further cell reaction. AMP cellular concentration is measured in $\text{mM}/\text{m}$ and it is considered as an important molecular biomarker for the cellular energetic state. ATP and AMP quantification was based on the enzyme coupling method presented in~\cite{ravera2013tricarboxylic}.
