...
\begin{enumerate*}[label=\roman*)]
	...
	\item weight (W).
\end{enumerate*}
Each sample of the dataset is represented by a vector of $d=165$ predictors.
...
Our final goal is predicting MS course evolution of  \RR and  \SP patients, hence the subjects with \PR and \PP forms will not be taken into account.
We considered all the patients with a minimum of $1$ time point (the most recently enrolled) up to $T=8$ time points for a total of $2699$ samples, of which $1220$ \RR and $1579$ \SP (see Figure~\ref{fig:patients}). As this is an ongoing project, the number of PwMS decreases with time. We expect to fill the gap of samples between \textit{Exam~$1$} and \textit{Exam~$8$} by the end of the funded study.
 ...
 \begin{figure}[h!]
	...
	\caption{An overview of the \PCO dataset used in this study. The left panel (a) shows a bar chart of the number of MS patients in each disease form  at different examinations. The right panel (b) presents a representation of the distribution of the total amount of acquisitions ($3137$), divided according to the disease form. \todo{break figure in two}}\label{fig:data}
\end{figure}
...
Secondly, as the missing data amount to $1.52\%$ of the entire dataset, we resort to the K-nearest neighbor data imputing strategy proposed in~\cite{troyanskaya2001missing}.
...
We split the dataset in three temporal chunks, namely {\em training}, {\em validation} and {\em test} sets, consisting of all samples collected at time points $t=1,2,3$, $t=4$ and $t=5,6,7,8$, respectively.
