...
\todo{add here details on PCOs}
\begin{itemize}
	\item[] {\sc \textbf{MFIS}} This is a $21$-items self-reported questionnaire typically administered in $5$ to $10$ minutes. \MFIS provides an assessment of the effects of fatigue in terms of physical, cognitive, psychosocial functioning and it is considered a valuable tool by clinicians.
	
	\item[] {\sc \textbf{HADS}} This is a $14$-items self-reported questionnaire typically administered in $2$ to $6$ minutes which aims at detecting clinically significant symptoms of depression and anxiety in patients. \HADS consists in $7$ questions for depression and the remaining $7$ for anxiety.
...
To tackle such issues, in this EDA we opted for a preliminary data standardization of the ordinal answers and a binary one-hot-encoding of the categorical ones, the latter increases the dimensionality of the samples, leading to $d=165$ variables in the dataset. The effect of the data standardization on the ordinal input variables is visually represented in Figure~\ref{fig:ms_boxplots}. As we can see, this preprocessing step allows to compare more easily the input features.
...
\begin{figure}[]
	...
	%	\hfill%
	\subfloat[]{%
		\includegraphics[trim={0 6cm 0 6cm}, width=0.5\textwidth]{part2/ms_ss_boxplot.png} \label{fig:ms_boxplots_post}
	}%
	\caption{The effect of the data standardization on the input ordinal \PCOs. The left panel (a) shows the distribution of the raw collected variables, whereas the right panel (b) shows the distribution of the same variables after the standardization step.}\label{fig:ms_boxplots}
\end{figure}
...
Following the EDA, we opted for a preliminary data standardization.
